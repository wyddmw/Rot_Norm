...
class KittiDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            # global rotation
            if self.opt.rotation_type == 'single':
                points, rotation = global_rotation_single(points, rotation=self.opt.rotation_angle/180*np.pi)
            elif self.opt.rotation_type == 'normal':
                points, rotation = global_rotation(points, rotation=np.pi/2)
            elif self.opt.rotation_type == 'pi_nodir':
                points, rotation = global_rotation_nodir(points, rotation=self.opt.rotation_angle/180*np.pi)
            elif self.opt.rotation_type == 'pi_aug':
