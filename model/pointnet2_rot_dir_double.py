...
import sys
sys.path.append('../')
from model.pointnet2_utils import PointNetSetAbstraction,PointNetSetAbstractionD


class PointCloudNormRotDirDouble(nn.Module):
    def __init__(self,in_channel=3):
        super().__init__()
        self.sa1 = PointNetSetAbstraction(npoint=1024, radius=0.2, nsample=32, in_channel=in_channel, mlp=[32, 64], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=256, radius=0.4, nsample=64, in_channel=64 + in_channel, mlp=[64, 128], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=128 + in_channel, mlp=[128, 1024], group_all=True)
